<?php
/**
 * heroCS-collab Theme Functions
 * Tema WordPress per CS Communication Agency con sistema collaborazioni
 *
 * @package heroCS-collab
 * @since 1.0.0
 * @author grimike
 */
...
// Theme constants
define('CS_THEME_NAME', 'heroCS-collab');
define('CS_THEME_VERSION', '1.3.0'); 
define('CS_THEME_DIR', get_template_directory());
define('CS_THEME_URI', get_template_directory_uri());
define('CS_AUTHOR_URL', 'https://archeoclubcaltagirone.it');
define('CS_TEXT_DOMAIN', 'herocs-collab');
...
function cs_theme_setup() {
    ...
    load_theme_textdomain(CS_TEXT_DOMAIN, CS_THEME_DIR . '/languages');
 ...
 // Aggiungi srcset per responsive images
    ...
    register_nav_menus(array(
        'primary' => esc_html__('Primary Menu', 'herocs'),
        'mobile' => esc_html__('Mobile Menu', 'herocs'),
        'footer' => esc_html__('Footer Menu', 'herocs'),
...
function cs_widgets_init() {
    ...
    register_sidebar(array(
        'name' => esc_html__('Sidebar', 'herocs'),
        'id' => 'sidebar-1',
        'description' => esc_html__('Add widgets here to appear in your sidebar.', 'herocs'),
    ...
    for ($i = 1; $i <= 3; $i++) {
        register_sidebar(array(
            'name' => sprintf(esc_html__('Footer %d', 'herocs'), $i),
            'id' => 'footer-' . $i,
            'description' => sprintf(esc_html__('Footer widget area %d', 'herocs'), $i),
...
function cs_excerpt_more($more) {
    return '... <a class="read-more" href="' . get_permalink() . '">' . esc_html__('Read More', 'herocs') . '</a>';
}
