...
function cs_get_social_links() {
    ...
    $networks = array(
        'facebook' => array(
            'label' => __('Facebook', 'cs-communication'),
            'icon' => 'facebook',
        ),
        'twitter' => array(
            'label' => __('Twitter', 'cs-communication'),
            'icon' => 'twitter',
        ),
        'linkedin' => array(
            'label' => __('LinkedIn', 'cs-communication'),
            'icon' => 'linkedin',
        ),
        'instagram' => array(
            'label' => __('Instagram', 'cs-communication'),
            'icon' => 'instagram',
        ),
        'youtube' => array(
            'label' => __('YouTube', 'cs-communication'),
            'icon' => 'youtube',
        ),
    );
...
function cs_breadcrumbs() {
    ...
    $home_title = __('Home', 'cs-communication');
    
    echo '<nav class="cs-breadcrumbs" aria-label="' . esc_attr__('Breadcrumb', 'cs-communication') . '">';
    ...
    if (is_single()) {
        ...
        echo '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="active">';
        echo '<span itemprop="name">' . esc_html__('Search Results', 'cs-communication') . '</span>';
        echo '<meta itemprop="position" content="' . $position . '" />';
        echo '</li>';
    } elseif (is_404()) {
        echo '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="active">';
        echo '<span itemprop="name">' . esc_html__('404 Not Found', 'cs-communication') . '</span>';
        echo '<meta itemprop="position" content="' . $position . '" />';
        echo '</li>';
    }
...
function cs_reading_time($post_id = null) {
    ...
    return sprintf(
        _n('%d minute read', '%d minutes read', $reading_time, 'cs-communication'),
        $reading_time
    );
}
...
function cs_pagination() {
    ...
    echo '<nav class="cs-pagination" role="navigation" aria-label="' . esc_attr__('Posts navigation', 'cs-communication') . '">';
    ...
    echo paginate_links(array(
        ...
        'prev_text' => '<span aria-hidden="true">&laquo;</span> ' . __('Previous', 'cs-communication'),
        'next_text' => __('Next', 'cs-communication') . ' <span aria-hidden="true">&raquo;</span>',
        'before_page_number' => '<span class="screen-reader-text">' . __('Page', 'cs-communication') . ' </span>',
        'type' => 'list',
    ));
...
function cs_post_meta_info() {
    ...
    echo '<span class="cs-post-views">' . sprintf(__('%s views', 'cs-communication'), cs_get_post_views()) . '</span>';
    echo '</div>';
}
